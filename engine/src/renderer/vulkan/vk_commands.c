...
b8 vk_command_buffer_record(VK_Context *context, VkCommandBuffer buffer, u32 image_index) {
    ...
    vkCmdBeginRenderPass(buffer, &render_pass_begin_info, VK_SUBPASS_CONTENTS_INLINE);
    {
        ...
        if (context->has_outline_pipelines) {
            u32 stencil_ref = 1;
            for (u32 i = 0; i < context->frame_mesh_count; i++) {
                ...
                f32 thickness = (fm->highlight == RL_HIGHLIGHT_HOVER)
                    ? context->outline_hover_thickness
                    : context->outline_selected_thickness;
                vec3 *color = (fm->highlight == RL_HIGHLIGHT_HOVER)
                    ? &context->highlight_hover_color
                    : &context->highlight_selected_color;

                if (thickness <= 0.0f) thickness = 0.03f;

                // Compute object-space center for imported meshes
                ...
                // Step 1: Stencil write — mark entity silhouette
                vkCmdBindPipeline(buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, context->outline_stencil_pipeline);
                vkCmdSetStencilReference(buffer, VK_STENCIL_FACE_FRONT_AND_BACK, stencil_ref);
                {
                    VK_MeshPushConstants pc;
                ...
                vkCmdBindPipeline(buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, context->outline_draw_pipeline);
                vkCmdSetStencilReference(buffer, VK_STENCIL_FACE_FRONT_AND_BACK, stencil_ref);
                {
                    VK_MeshPushConstants pc;
                    glm_mat4_copy(fm->model, pc.model);
                    glm_vec3_copy(*color, pc.material_params);
                    pc.material_params[3] = thickness;
                    glm_vec3_copy(obj_center, pc.obj_center);
                    pc.obj_center[3] = 0.0f;
                    vkCmdPushConstants(buffer, context->pipeline_layout,
                        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                        0, sizeof(VK_MeshPushConstants), &pc);
                    vkCmdDraw(buffer, context->cube_vertex_count, 1, 0, 0);
