...
b32 msdf_load_font_codepoints(const char *path, const u32 *codepoints, u32 count, rl_font *out_font) {
    ...
    // Build charset from the explicit codepoint list
    Charset charset;
    for (u32 i = 0; i < count; i++) {
        msdfgen::GlyphIndex gi;
        if (msdfgen::getGlyphIndex(gi, font, codepoints[i]))
    ...
    msdfgen::BitmapRef<byte, 4> bitmap(out_font->atlas.data, width, height);
    storage.get(0, 0, bitmap);

    RL_INFO("msdf_load_font_codepoints: loaded %u glyphs, atlas %dx%d", out_font->glyph_count, width, height);
